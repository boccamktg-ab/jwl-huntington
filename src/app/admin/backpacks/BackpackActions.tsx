...
type Props = {
  ...
  action: string
  label: string
  variant: 'green' | 'gray' | 'blue' | 'ghost' | 'danger'
...
export default function BackpackActions({ id, action, label, variant, confirmedCount = 0 }: Props) {
  ...
  const cls = {
    ...
    danger: 'text-xs text-red-400 hover:text-red-600 hover:underline',
  }[variant]
  ...
  async function run() {
    ...
    if (action === 'delete') { setConfirming(true); return }
    await execute()
  }
  ...
  if (confirming) {
    return (
      <div className="inline-flex items-center gap-1">
        <button onClick={execute} disabled={loading} className="text-xs px-2 py-1 bg-red-600 text-white rounded hover:bg-red-700 disabled:opacity-50">
