...
import { sendEmail, emailPaymentNudge } from '@/lib/email'
...
export async function POST(request: NextRequest) {
  ...
  if (!cheddarUpUrl) {
    return NextResponse.json({ error: 'CheddarUp URL not configured in app settings.' }, { status: 400 })
  }
  ...
  for (const m of pending) {
    const { subject, html } = emailPaymentNudge(m.name, cheddarUpUrl)
